...
void CalculateDifferentialSensitivity(TH1D *hTau)
{
    ...
    tanFoV = tan(dFoV/180.*pi); // was dFoV in the place of 89
    //dFoVBelow = asin(REarth/(REarth+DetectorAltitude[iConfig]));
    dFoVBelow =  3/180.*pi; 
    iMirrorSize = 2;
    dMinimumNumberPhotoelectrons = dThreshold[iMirrorSize]/dMirrorA[iMirrorSize]; 
          ...
          if(dAcceptance>1)
           {
              ...
              cout<<"Energy "<<dLogE<<" acceptance:  "<<dAcceptance<<" nuFnu: "<<dnuFnu<<endl;
              grSensitivity->SetPoint(n,pow(10,dLogE),dnuFnu);
              cDiffSensitivity->cd();
              grSensitivity->Draw("alp");
  ...
  TString Filename;
  Filename.Form("SensitivityResults/DifferentialSensitivityTrinity_NuTauSim_%ikmAboveGround_%0.0fsqrmMirror_%0.1fdegUpperFoV_%0.1fdegLowerFoV_%0.1fdegMinShowerLength.root",iConfig,dMirrorA[iMirrorSize],dFoV,dFoVBelow/pi*180.,dMinLength);
  cDiffSensitivity->SaveAs(Filename.Data());
  Filename.Form("SensitivityResults/AcceptanceTrinity_NuTauSim_%ikmAboveGround_%0.0fsqrmMirror_%0.1fdegUpperFoV_%0.1fdegLowerFoV_%0.1fdegMinShowerLength.root",iConfig,dMirrorA[iMirrorSize],dFoV,dFoVBelow/pi*180.,dMinLength);
  cAcceptance->SaveAs(Filename.Data());
  Filename.Form("SensitivityResults/TriggeredAzimuthTrinity_NuTauSim_%ikmAboveGround_%0.0fsqrmMirror_%0.1fdegUpperFoV_%0.1fdegLowerFoV_%0.1fdegMinShowerLength.root",iConfig,dMirrorA[iMirrorSize],dFoV,dFoVBelow/pi*180.,dMinLength);
...
for(int i=0;i<hTau->GetNbinsX();i++)
{
  ...
  {
   //  cout<<"Energy "<<E<<endl;
   Double_t P = PEtauNoEnergyLoss(100,Etau,Enu,hTau) ;

...
void GetTauDistributionNEL(TH1D *hTauSpec, Double_t d, Double_t Enumin = 1e9, Double_t Enumax = 3.16e9)
{
   hTauSpec->Reset(); //get the energy spectrum of taus coming out of the Earth, starting with nus in the range expmin expMax
   //int nEnuSteps = 20;
   //Double_t DeltaEnu = (Enumax - Enumin)/nEnuSteps;
          ...
          if(bMonoNu) //if we want to simulate only monoenergetic neutrinos (only good for acceptance calculations
            {
               ...
               //if(Etau<=0.8*Enu) //0.8 because I need to account for part of the energy being transferred to the also produced neutrinos
                 {
                    Double_t Enu = pow(10,(log10(Enumin)+log10(Enumax))*0.5);
                    Double_t P = PEtauNoEnergyLoss(d,Etau,Enu,hTauSpec);
                    hTauSpec->Fill(Etau,P);
                 } 
            }
          else
                  ...
                  //if(Etau<=0.8*Enu) //0.8 because I need to account for part of the energy being transferred to the also produced neutrinos
                    {
                      Double_t P = 0;
                      //if(Enu==Enumin)  //mod
                      P = PEtauNoEnergyLoss(d,Etau,Enu[n],hTauSpec);
                      //if(Enu==Enumin) //mod
                      //cout<<P<<"  "<<d<<"  "<<Etau<<" "<<Enu[n]<<"  "<<EnuWeight[n]<<endl; //mod
                      //P *= DeltaEnu/(Enumax-Enumin); 
                      P *= EnuWeight[n];
...
Double_t CalculateAcceptanceNEL(Double_t dMinEnu, Double_t dMaxEnu,TGraph *grDiffAcceptance,TH1D *hTau)
{
            ...
            while(azimuth<MaxAzimuth) // loop over azimuth
              {
                 ...
                 //cout<<dEarth<<"  "<<dMinEnu<<"  "<<dMaxEnu<<endl;
                 GetTauDistributionNEL(hTau,dEarth,dMinEnu,dMaxEnu);
                 //cout<<"done GetTau"<<endl;
...
void CalculateIntegralSensitivity0(TH1D *hTau){
	...
	while(Ei<Eh ){
		double E = pow(10, Ei) ;
		double dE = pow(10, Ei+dE_log) - E ;
		double accept = CalculateAcceptance(Ei, Ei+dE_log, grDiffAcceptance,hTau) ;
		dIntegr += accept * pow(E, -gam) * dE ;
		Ei += dE_log ;
	}
	double Fo = 1 / (t * pow(Eo, gam) *  dIntegr) ;
	Ei = 6 ;
	int n = 0 ;
	while(Ei<Eh ){
		double E = pow(10, Ei) ;
		double F = Fo * pow(E/Eo, -gam );
        grSensitivity->SetPoint(n, E, F * E * E);
        n++ ;
		Ei += dE_log ;
    ...
    while(dLogE<=logEmax)
       {
          ...
          Double_t dAcceptanceNEL = CalculateAcceptanceNEL(dLogE-dHalfEnergyBinWidth,dLogE+dHalfEnergyBinWidth,grDiffAcceptance,hTau) ;
          if(dAcceptance>1)
           {
              ...
              Double_t dnuFnuNEL = 3 * 2.44 * pow(10,dLogE*2) / dAcceptanceNEL / dExposure / (pow(10,dLogE+dHalfEnergyBinWidth)- pow(10,dLogE-dHalfEnergyBinWidth));
//              cout<<"Energy "<<dLogE-dHalfEnergyBinWidth<<" to "<<dLogE+dHalfEnergyBinWidth<<" acceptance:  "<<dAcceptance<<" nuFnu: "<<dnuFnu<<" for power law with index -"<<nuIndex<<endl;
              cout << "dnuFnu:" << dnuFnu << endl;
              cout << "dnuFnuNEL: "<< dnuFnuNEL << endl;
              grSensitivity->SetPoint(n,pow(10,dLogE),dnuFnu) ;
              cDiffSensitivity->cd() ;
              grSensitivity->Draw("alp") ;
              ...
              dAcceptanceNEL = CalculateAcceptanceNEL(dLogE-dHalfEnergyBinWidth,dLogE+dHalfEnergyBinWidth,grDiffAcceptance,hTau) ;
              ...
              dnuFnuNEL = 3 * 2.44 / dAcceptanceNEL / dExposure / log(10) / (2*dHalfEnergyBinWidth) * pow(10,dLogE) ;
              cout<<"NuCommunity Definition: "<<dnuFnu<<" acceptance at center energy: "<<dAcceptance<<endl ;
              cout<<"NuCommunity Definition NEL: "<<dnuFnuNEL<<" acceptance at center energy: "<<dAcceptanceNEL<<endl ;
